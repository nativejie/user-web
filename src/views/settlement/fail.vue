<template>
    <div class="order_placed">
      <p class="suc_img"><i class="el-icon-error"></i></p>
      <p style="font-size: 24px; font-weight: 600; line-height: 32px">
        {{ elementContentList.portal_settlement_text9 || $t("settlement.text9") }}
      </p>
      <div class="order_card">
        <!-- <p class="order_title">{{ "Shipping to" }}</p>
          <p>{{ settlemenData.addressInfo.name }}</p>
          <p class="flex_b_c">
            <span>{{
              settlemenData.addressInfo.detailAddress +
              "," +
              settlemenData.addressInfo.city +
              "," +
              settlemenData.addressInfo.country
            }}</span>
  ...
  export default {
    ...
    methods: {
      handleToOrder () {
        ...
        // this.$router.push({ name: 'order', params: { orderId: this.settlemenData.payResult.orderId } })
